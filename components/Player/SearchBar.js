...
function SingleCard({ item, isLoading = false }) {
  ...
  return (
    <div
      key={item?.name}
      className={'flex items-center p-4 border-b last:border-b-0 cursor-pointer' + (isLoading ? ' animate-pulse' : '')}
    ...
    >
      <img src={item?.screenshot_url} alt={item?.attributes?.app_name} className="w-36 h-24 rounded-m mr-3" />
...
function SearchBar() {
  ...
  const [showQueryHTML, setShowQueryHTML] = useState('');
  ...
  const handleSearch = async (e) => {
    ...
    // If there're any @ symbols, replace them with spaces, wrap them in red spans
    // and set the innerHTML of the query box to the new string
    let queryHTML = value.replace(/@/g, '<span class="text-red-500">@</span>');
    setShowQueryHTML(queryHTML);
  ...
  return (
    <div className="relative w-full">
      ...
      <div
        className="block w-full p-4 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 absolute top-0 left-0"
        contentEditable={true}
        suppressContentEditableWarning={true}
        onInput={(e) => {
          const value = e.currentTarget.textContent;
          handleSearch({ target: { value: value } });  // Forward the value to handleSearch
          inputRef.current.value = value;  // Set the value of the real input
        }}
        dangerouslySetInnerHTML={{ __html: showQueryHTML }}
      ></div>
      ...
      <input
        ...
        className="opacity-0 absolute top-0 left-0 cursor-text"  // Make it invisible but still functional
